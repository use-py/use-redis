...
import redis
import pytest
from usepy_plugin_redis.stream import RedisStreamStore, RedisStreamMessage
...
class TestRedisStreamMessage:
    ...
    def test_to_json(self):
        ...
        assert json.loads(message_json) == {'id': '1', 'body': {'foo': 'bar'},
                                            'stream': 'test_stream',
                                            'group': 'test_group'}
...
class TestRedisStreamStore:
    ...
    def test_claim_old_pending_messages(self, redis_stream_store):
        ...
        messages = redis_stream_store.claim_old_pending_messages(consumer, count=1, min_idle_time=0)
    ...
    def test_consume_messages_with_idle_time(self, redis_stream_store):
        ...
        messages = redis_stream_store.consume(consumer, prefetch=1, force_claim=True, redeliver_timeout=500)
        assert messages == []
        time.sleep(1)
        messages = redis_stream_store.consume(consumer, prefetch=1, force_claim=True, redeliver_timeout=500)
        assert isinstance(messages, list)
